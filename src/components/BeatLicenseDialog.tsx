...
export function BeatLicenseDialog({ beat, producerName, children }: BeatLicenseDialogProps) {
  ...
  const handlePurchase = async (licenseType: string) => {
    if (!user) {
      actionToast.info("Sign in to license beats");
      navigate("/login");
      return;
    }
    if (user.id === beat.producer_id) {
      actionToast.error("You can't license your own beat");
      return;
    }
    ...
    try {
      ...
      if (price.bak <= 0) {
        actionToast.error("This tier isn't available");
        return;
      }
      ...
      if (!wallet || wallet.balance < price.bak) {
        actionToast.error("Not enough BAKCoin", `You need ${price.bak} BAK to license this beat.`);
        navigate("/buy-coins");
        return;
      }
      ...
      actionToast.success(`${tier.label} unlocked`, "Head to your downloads to grab the files.");
      setOpen(false);
    } catch (err: any) {
      actionToast.error("Purchase failed", err?.message || "Please try again.");
    } finally {
      setPurchasing(null);
    }
  };
  ...
  if (beat.is_free) {
    return (
      <ResponsiveModal
        ...
        trigger={children}
        title="Free beat"
        description={`"${beat.title}" by ${producerName} is free to use. Download and create!`}
      >
        <PressableButton
          hapticPattern="success"
          className="w-full"
          onClick={() => actionToast.success("Download started")}
        >
  ...
  return (
    <ResponsiveModal
      ...
      description={`Choose a license tier from ${producerName}.`}
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-3 pt-2">
        {LICENSE_TIERS.map((tier) => {
          ...
          return (
            <Card
              key={tier.type}
              className={`relative card-base ${!isAvailable ? "opacity-50" : "hover:border-primary/50"} transition-all`}
            >
              {tier.type === "premium" && (
              ...
              <CardContent className="p-4 space-y-3">
                ...
                <PressableButton
                  variant={tier.type === "premium" ? "default" : "outline"}
                  size="sm"
